...
/// Trả về zone bị chồng (nếu có), null nếu ok
GeoZone? findOverlappingZone({
  required GeoZone candidate,
  required List<GeoZone> existing,
  double bufferM = 5, // khoảng cách an toàn thêm (tuỳ bạn)
}) {
  for (final z in existing) {
    if (z.id == candidate.id) continue; // bỏ qua chính nó khi edit
    final d = _haversineMeters(candidate.lat, candidate.lng, z.lat, z.lng);
    if (d < (candidate.radiusM + z.radiusM + bufferM)) return z;
  }
  return null;
}